/*
 * Cropper.js 1.6.2 — vendored for Boxlet under the SPEC §3 rule (PLAN.md D-026).
 *
 * Source: https://cdnjs.cloudflare.com/ajax/libs/cropperjs/1.6.2/cropper.min.js
 * Upstream: https://github.com/fengyuanchen/cropperjs — MIT, Chen Fengyuan.
 *
 * Do not edit. To update, fetch the new version, put its number and URL here, and
 * change the row in docs/SPEC.md §3 so the two never disagree.
 *
 * It earns its place: cropping by hand needs a draggable, resizable box with
 * ratio locking and touch support, which is a lot of fiddly geometry to get right
 * and nothing the owner would thank us for reimplementing.
 *
 * CSP: it writes only <div>s through innerHTML and sets styles via style.cssText,
 * which is CSSOM and not policed. It injects no stylesheet, so the admin policy
 * stays as it is — unlike Trix, which is why admin-richtext.css exists.
 */
/*!
 * Cropper.js v1.6.2
