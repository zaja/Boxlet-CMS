...
```
/{path}                       page in the primary locale
/                             home page in the primary locale
/{locale}/{path}              page in an additional locale
...
**`{path}` is the page's slug under its parents' slugs**, `usluge/web-dizajn` (PLAN.md
D-129, changed deliberately before v0.1; it was one segment). The home page adds no
segment, so its children sit at the top level. A slug stays unique in its locale, so the
last segment finds the page. A request whose path in front of it is wrong, after a parent
was renamed or a page moved, gets a 301 to the right one, query kept, but only when every
segment in front is a slug a page of that locale has or had. Any other prefix is a 404,
so `/de/hello` with `de` not enabled stays one. The page route never matches under a
system word, so `/admin/...` is always the admin's. The path is not
stored: `Url::page()` works it out from the page tree, once per request. A page below the
top level carries `BreadcrumbList` JSON-LD naming its published ancestors.
...
soft 404. All ISO 639-1 codes are reserved as page slugs and rejected on save, so a page
can never collide with a locale enabled later. So are the system words (`Slug::SYSTEM`),
`download` and `sitemap` among them, which routes of their own answer.
